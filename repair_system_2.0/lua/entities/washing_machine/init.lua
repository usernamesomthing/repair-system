...
if(self.hitsneeded >= 99) then return; end 
	...
	if(self.hitsneeded >= 99) then 
		self.repair = true
			until_restore_wash = CurTime() + 480
			DarkRP.notify(player, 3, 6, "You repaired a washing machine! You got Payed $"..moneyaddedforrepair)
...
self:SetNWBool( "repair_state_washing", false )
self:SetNWString('repair_precent_washing',precents.. "% Repaired...")
end

if self.repair == true then
self:SetNWBool( "repair_state_washing", true )
end

if self.repair == true then
if CurTime() < until_restore_wash then return end
self.hitsneeded = math.random(  0,15  )
self.repair = false
self:SetNWBool( "repair_state_washing", false )
end
