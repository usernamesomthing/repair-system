...
self:SetModel( "models/props_interiors/toilet_b.mdl" )
...
if(self.hitsneeded >= 99) then return; end 
	...
	if(self.hitsneeded >= 99) then 
		self.repair = true
			until_restore_toliet = CurTime() + 480
			DarkRP.notify(player, 3, 6, "You repaired a toilet! You got Payed $"..moneyaddedforrepair)
...
self:SetNWBool( "repair_state_toliet", false )
self:SetNWString('repair_precent_toliet',precents.. "% Repaired...")
end

if self.repair == true then
self:SetNWBool( "repair_state_toliet", true )
end

if self.repair == true then
if CurTime() < until_restore_toliet then return end
self.hitsneeded = math.random(  0,15  )
self.repair = false
self:SetNWBool( "repair_state_toliet", false )
end
