...
DarkRP.notify(ply, 3, 6, "You need to repair this!")

end

if self.repair == true then
	if activator:IsPlayer() then
    ply:ChatPrint("You type on a keyboard.")
	end
end
end
...
if(self.hitsneeded >= 99) then return; end 
	...
	if(self.hitsneeded >= 99) then 
		self.repair = true
			until_restore_cons = CurTime() + 480
			DarkRP.notify(player, 3, 6, "You repaired a console! You got Payed $"..moneyaddedforrepair)
...
if self.repair == false then
self:SetNWBool( "repair_state_cons", false )
self:SetNWString('repair_precent_cons',precents.. "% Repaired...")
end

if self.repair == true then
self:SetNWBool( "repair_state_cons", true )
end

if self.repair == true then
if CurTime() < until_restore_cons then return end
self.hitsneeded = math.random(  0,15  )
self.repair = false
self:SetNWBool( "repair_state_cons", false )
end
