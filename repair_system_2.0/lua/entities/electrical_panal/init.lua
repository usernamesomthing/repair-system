...
if(self.hitsneeded >= 99) then return; end 
	...
	if(self.hitsneeded >= 99) then 
		self.repair = true
			until_restore_elec_panal = CurTime() + 480
			DarkRP.notify(player, 3, 6, "You repaired a eletrical box! You got Payed $"..moneyaddedforrepair)
...
self:SetNWBool( "repair_state_elec_pan", false )
self:SetNWString('repair_precent_elec_pan',precents.. "% Repaired...")
end

if self.repair == true then
self:SetNWBool( "repair_state_elec_pan", true )
end

if self.repair == true then
if CurTime() < until_restore_elec_panal then return end
self.hitsneeded = math.random(  0,15  )
self.repair = false
self:SetNWBool( "repair_state_elec_pan", false )
end
