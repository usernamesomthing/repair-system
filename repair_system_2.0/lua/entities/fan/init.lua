...
self:SetModel( "models/props_office/deskfan.mdl" )
...
if(self.hitsneeded >= 99) then return; end 
	...
	if(self.hitsneeded >= 99) then 
		self.repair = true
			until_restore_fan = CurTime() + 480
			DarkRP.notify(player, 3, 6, "You repaired a fan! You got Payed $"..moneyaddedforrepair)
...
self:SetNWBool( "repair_state_fan", false )
self:SetNWString('repair_precent_fan',precents.. "% Repaired...")
end

if self.repair == true then
self:SetNWBool( "repair_state_fan", true )
end

if self.repair == true then
if CurTime() < until_restore_fan then return end
self.hitsneeded = math.random(  0,15  )
self.repair = false
self:SetNWBool( "repair_state_fan", false )
end
